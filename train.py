...
class joystickEventSender(threading.Thread):
    ...
    running = 1
    JoystickData = []
    def __init__(self, practice_json):
        self.practice_json = practice_json
        pygame.init()
    ...
    def run(self):
        for tries in range(10):
            ...
            index_move = {"Left": 0, "Right" : 0, "Up" : 1, "Down" : 1, "Start" : 2, "Select" : 2, "B" : 3, "X": 4, "A" : 5, "Y" : 6, "L" : 7, "R" :8, "" : 0}
            goal_states = []
            setup  = json.load(open(self.practice_json, 'r'))
            ...
            while sequence_incomplete:
                ...
                if self.JoystickData == goal_states[idx]:
                    ...
                    step_time_value = time.clock() - step_timer
                    tracking[time_index]["step_time"].append(step_time_value)
                    try:
                        if abs(step_time_value - timing[idx]) > .035:
                            print ("You pressed", sequence[idx], "a bit too soon", step_time_value,"vs",  timing[idx])
                    except:
                        pass
                    idx += 1
                    step_timer = time.clock()
    ...
    #                    print ("It took you ", time.clock() - start_time, "seconds from the start to end of the move doing it correctly.")
                        ...
                        for i in ["count", "sequence_time", "total_time"]:
